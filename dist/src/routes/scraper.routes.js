...
const ScrapedCarPrice_1 = __importDefault(require("../models/ScrapedCarPrice"));
 ...
 */
function isDataStale(scrapedAt) {
    const now = new Date();
    const diffInHours = (now.getTime() - scrapedAt.getTime()) / (1000 * 60 * 60);
...
router.get("/", async (req, res) => {
    try {
        ...
        // Build query
        const query = { staticData: false };
        if (brand)
            query.brand = { $regex: brand, $options: "i" };
        if (model)
            query.model = { $regex: model, $options: "i" };
        // Get latest prices for each brand-model-variant combination
        const prices = await ScrapedCarPrice_1.default.aggregate([
            { $match: query },
            { $sort: { scrapedAt: -1 } },
            {
                $group: {
        ...
        if (formattedPrices.length === 0 || formattedPrices.every((p) => p.isStale)) {
            ...
            const staticPrices = await ScrapedCarPrice_1.default.aggregate([
                { $match: staticQuery },
                { $sort: { scrapedAt: -1 } },
                {
                    $group: {
            ...
            const staticFormattedPrices = staticPrices.map((item) => ({
                brand: item._id.brand,
                model: item._id.model,
                variant: item._id.variant,
                price: item.price,
...
router.get("/:brand", async (req, res) => {
    try {
        ...
        const formattedPrices = prices.map((item) => ({
            brand: brand && typeof brand === 'string' ? brand.toUpperCase() : 'UNKNOWN',
            model: item._id.model,
            variant: item._id.variant,
            price: item.price,
            currency: item.currency,
...
router.get("/status/health", async (req, res) => {
    try {
        ...
        let lastScraped = await ScrapedCarPrice_1.default.findOne({ staticData: false }, {}, { sort: { scrapedAt: -1 } });
        if (!lastScraped) {
            // Fall back to checking static data
            lastScraped = await ScrapedCarPrice_1.default.findOne({ staticData: true }, {}, { sort: { scrapedAt: -1 } });
        }
        if (!lastScraped) {
            return res.json({
                success: true,
        ...
        const hoursAgo = Math.round((new Date().getTime() - lastScraped.scrapedAt.getTime()) / (1000 * 60 * 60));
        res.json({
            success: true,
